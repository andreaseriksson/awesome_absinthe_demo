defmodule AwesomeAbsintheDemoWeb.Schema.DeleteThingTest do
  use AwesomeAbsintheDemoWeb.ConnCase, async: false
  import AwesomeAbsintheDemo.GraphqlTestHelpers
  ...
  describe "when logged in" do
    ...
    test "delete thing with valid credentials", %{
      ...
      assert thing_id == "#{id}"
      assert AwesomeAbsintheDemo.Things.list_things(user) == []
    end
    ...
    test "delete thing when id is wrong", %{conn: conn, jwt: jwt, user: user} do
      ...
      assert [_] = AwesomeAbsintheDemo.Things.list_things(user)
    end
  end
end
